...
fn lock_with_visibility(app: &AppHandle, show_lock: bool) {
    ...
    *state.hidden.lock().unwrap() = hidden;
    if show_lock {
        show_lock_window(app);
    } else if let Some(window) = app.get_webview_window("lock") {
        let _ = window.hide();
    }
}

pub fn lock_now(app: &AppHandle) {
    lock_with_visibility(app, true);
}

/// Lock while minimizing to the tray. The unlock prompt is deferred until the
/// user deliberately restores WhatsNow.
pub fn lock_to_tray(app: &AppHandle) {
    lock_with_visibility(app, false);
}
...
pub fn show_lock_window(app: &AppHandle) {
    ...
    if let Ok(win) = built {
        let _ = win.set_theme(crate::settings::load(app).theme.native_theme());
