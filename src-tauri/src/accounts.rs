...
pub struct Account {
    pub id: String,
    pub name: String,
    /// WhatsApp's own Profile > Name value, discovered read-only by the page bridge.
    /// It drives the native window title without replacing the user's optional
    /// WhatsNow account label.
...
/// Store WhatsApp's own display name for the account. Returns whether it changed.
pub fn set_profile_name(
    f: &mut AccountsFile,
    id: &str,
    profile_name: &str,
) -> Result<bool, String> {
    let Some(account) = f.accounts.iter_mut().find(|account| account.id == id) else {
        return Err(format!("unknown account: {id}"));
    };
    if account.profile_name.as_deref() == Some(profile_name) {
        return Ok(false);
    }
    account.profile_name = Some(profile_name.to_string());
    Ok(true)
}

/// Prefer WhatsApp's live profile name for native chrome, falling back to the
/// optional WhatsNow account label until the profile has been discovered.
pub fn title_name(account: &Account) -> &str {
    account.profile_name.as_deref().unwrap_or(&account.name)
}
...
mod tests {
    ...
    fn default_file_has_single_default_account() {
        ...
        assert_eq!(a.profile_name, None);
        assert_eq!(a.order, 0);
        assert_eq!(a.store_uuid, None);
        assert!(a.notifications_enabled);
    ...
    #[test]
    fn profile_name_is_separate_and_drives_title() {
        let mut f = AccountsFile::default();
        assert_eq!(title_name(&f.accounts[0]), "WhatsApp");
        assert!(set_profile_name(&mut f, "default", "Ben").unwrap());
        assert_eq!(f.accounts[0].name, "WhatsApp");
        assert_eq!(title_name(&f.accounts[0]), "Ben");
        assert!(!set_profile_name(&mut f, "default", "Ben").unwrap());
    }

    #[test]
    fn account_notifications_can_be_muted_and_unmuted() {
        let mut f = AccountsFile::default();
        set_notifications_enabled(&mut f, "default", false).unwrap();
        assert!(!f.accounts[0].notifications_enabled);
        set_notifications_enabled(&mut f, "default", true).unwrap();
        assert!(f.accounts[0].notifications_enabled);
    }
    ...
    fn partial_json_fills_defaults() {
        ...
        assert_eq!(f.accounts[0].profile_name, None);
        assert!(f.accounts[0].notifications_enabled);
